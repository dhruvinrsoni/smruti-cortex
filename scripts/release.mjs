...
if (RESUME) {
  ...
  if (!state.releaseCommitPresent) {
    ...
    // Refresh the public quality snapshot for the resumed release too.
    // Best-effort: if it fails, fall back to whatever's already on disk.
    try {
      run(`node scripts/build-dashboard.mjs --out docs/quality-report --snapshot-version ${state.targetTag}`);
    } catch (e) {
      console.log(`${YELLOW}⚠️  Quality snapshot refresh failed (non-blocking): ${e.message}${RESET}`);
    }
    if (existsSync(resolve(ROOT, 'docs/quality-report/index.html'))) commitFiles.push('docs/quality-report/index.html');
    if (existsSync(resolve(ROOT, 'docs/quality-report/summary.json'))) commitFiles.push('docs/quality-report/summary.json');
...
// Refresh the public Quality Report snapshot at docs/quality-report/.
// This is the *only* moment the snapshot is auto-regenerated by the pipeline
// — every other refresh is operator-driven via `npm run dashboard refresh`.
// Coverage data was produced by the ship-check gate (Step 2's `verify --release`)
// and dist/ was rebuilt with the new version in Step 5, so inputs are fresh.
// Non-fatal: if the dashboard fails to render (e.g. unexpected coverage layout)
// we log a warning and proceed — the release should not be blocked by a static
// snapshot, and the artifact dashboard remains the source of truth.
try {
  run(`node scripts/build-dashboard.mjs --out docs/quality-report --snapshot-version v${newVersion}`);
  console.log(`${GREEN}✅ Quality snapshot refreshed at docs/quality-report/ (stamped v${newVersion})${RESET}`);
} catch (e) {
  console.log(`${YELLOW}⚠️  Quality snapshot refresh failed (non-blocking): ${e.message}${RESET}`);
  console.log(`${YELLOW}   The release will continue. Run \`npm run dashboard refresh\` after the release to fix.${RESET}`);
}
...
// Snapshot files exist if Step 6's refresh succeeded. Add them only if present
// so a failed refresh doesn't break the commit.
const snapshotIndex = resolve(ROOT, 'docs/quality-report/index.html');
const snapshotSummary = resolve(ROOT, 'docs/quality-report/summary.json');
if (existsSync(snapshotIndex)) commitFiles.push('docs/quality-report/index.html');
if (existsSync(snapshotSummary)) commitFiles.push('docs/quality-report/summary.json');
run(`git add ${commitFiles.join(' ')}`);
