...
| `nfr-report.yml` | 2026-05-01 | Replaced by `health-check.yml`. Dashboard generation extracted into `scripts/build-dashboard.mjs`. The dashboard is now bundled into the run artifact (download `smruti-cortex-health-bundle` and open `dashboard/index.html`) — explicitly NOT published to GitHub Pages so the marketing site + privacy URL aren't single-pointed-of-failure on the workflow. PR coverage delta comment moved into health-check. |
